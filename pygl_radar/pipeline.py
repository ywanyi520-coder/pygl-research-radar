...
def run_radar(
    ...
    *,
    ...
    for paper in triaged:
        ...
        review_paper(paper, review_client)
    ...
    message = render_wechat_message(recommended, stats, report_url=str(markdown_path))
    push_result = _notifier(dry_run, notifier).send(message)
    if push_result.ok and not dry_run:
        seen.mark(recommended)
        seen.save()
    return RunResult(recommended, stats, markdown_path, html_path, json_path, push_result)
